...
object OmeZarr:
    ...
    /** Key in 'axes' mapping for which the value encodes ZARR dimension unit of measure, e.g. "micrometer" */
    ...
    object DimensionName:
        ...
        def liftOption(opt: Option[String]): Option[DimensionName] = opt.map(liftString)
    ...
    /**
      ...
      *
      * @param targetDescription The contextual information with which to enrich 
      *     potential error message
      * @return See above
      */
    private def safeCast[O](targetDescription: String): Any => Either[String, O] = i =>
        Try{ i.asInstanceOf[O] }.toEither.leftMap {
            case e: ClassCastException => s"Could not complete type cast ($targetDescription): ${e.getMessage}"
    ...
    final case class IndexMapping private(
        ...
        def buildOriginIndex(coordinate: OmeZarrStandardCoordinate): Array[Int] = Array(
    ...
    object IndexMapping:
        ...
        /**
          * Attempt to get positional index for each OME-ZARR dimension, 
          * according to the metadata associated with the given ZARR group.
          *
          * @param zg The ZARR group with metadata to parse
          * @return Either a [[scala.util.Left]]-wrapped collection of error/problem 
          *     messages, or a [[scala.util.Right]]-wrapped parsed mapping
          */
        def fromZarrGroup(zg: ZarrGroup): Either[NonEmptyList[String], IndexMapping] = 
            getRawAxesMaps(zg)
                .leftMap(NonEmptyList.one)
                .flatMap(getOmeZarrDimensionMap)

        def fromDimensions(dims: NonEmptyList[OmeZarrDimension]): Either[NonEmptyList[String], IndexMapping] = {
            val indexedDims = dims.zipWithIndex.map((d, i) => d -> NonnegativeInt.unsafe(i))
            if indexedDims.length =!= 5 
            then NonEmptyList.one(s"Expected 5 dimensions but got ${indexedDims.length}").asLeft 
            else
                val tNel = getUniqueIndexFor("time")(_.isInstanceOf[TimepointDimension])(indexedDims).toValidatedNel
                val cNel = getUniqueIndexFor("channel")(_.isInstanceOf[ChannelDimension])(indexedDims).toValidatedNel
                val zNel = getUniqueIndexFor("z")(_.isInstanceOf[ZDimension])(indexedDims).toValidatedNel
                val yNel = getUniqueIndexFor("y")(_.isInstanceOf[YDimension])(indexedDims).toValidatedNel
                val xNel = getUniqueIndexFor("x")(_.isInstanceOf[XDimension])(indexedDims).toValidatedNel
                (tNel, cNel, zNel, yNel, xNel).tupled.toEither.flatMap{ (t, c, z, y, x) => 
                    val reps = List(t, c, z, y, x).groupBy(identity).view.mapValues(_.length).filter(_._2 > 1).toMap
                    reps.isEmpty.either(NonEmptyList.one(s"Repeated dimension index counts: $reps"), IndexMapping(t, c, z, y, x))
                }
        }

        private def getUniqueIndexFor(targetName: String)(isMatch: OmeZarrDimension => Boolean): NonEmptyList[(OmeZarrDimension, NonnegativeInt)] => Either[String, NonnegativeInt] = 
            dims => (
    ...
    private def getOmeZarrDimensionMap(rawAxes: List[Map[String, String]]): Either[NonEmptyList[String], IndexMapping] = for {
    ...
    /**
      ...
      *
      *
      * @param m The metadata to parse
      * @return Either a [[scala.util.Left]]-wrapped nonempty list of explanations of why 
      *     the metadata parse failed, or a [[scala.util.Right]]-wrapped dimension entity
      */
    private def parseDimension(m: Map[String, String]): Either[NonEmptyList[String], OmeZarrDimension] = {
        ...
        (nameOptNel, typeNel, unitOptNel).tupled.toEither.flatMap{
            ...
            case (_, typename@(`timeName` | `channelName`), Some(unitName)) => 
                NonEmptyList.one(s"Cannot have unit name for dimension type '$typename'; got: '$unitName'").asLeft
            ...
            case (dimOpt, "space", unitOpt) => 
                val buildNel: ValidatedNel[String, BuildSpatial] = 
                    dimOpt match {
                        ...
                        case Some(dimname) => s"Illegal name for spatial dimension: $dimname".invalidNel[BuildSpatial]
                        case None => "To decode spatial dimension, name is required".invalidNel[BuildSpatial]
                    }
                val unitNel: ValidatedNel[String, UnitsLength] = unitOpt
                    .toRight("For spatial dimension, unit is required")
                    .flatMap(unitName => UnitsLength.fromString(unitName).toRight(s"Cannot decode as length units name: $unitName"))
                    .toValidatedNel
    ...
    /**
      ...
      *         "scale" : [ 1.0, 1.0, 0.18999999999999995, 0.07059082892416223, 0.07059082892416223 ],
      *         "type" : "scale"
      *       } ]
      *     } ],
      *     "version" : "0.4"
      ...
      * }
      *
      * @param zg The ZARR group for which associated axis/dimension metadata is desired
      * @return Either a [[scala.util.Left]]-wrapped explanation of why the metadata parse failed, 
      *     or a [[scala.util.Right]]-wrapped list of raw (string-to-string) mappings, each 
      *     representing a single axis/dimension
      */
    private def getRawAxesMaps(zg: ZarrGroup): Either[String, List[Map[String, String]]] = for {
        attrs <- (zg.getAttributes() != null).either("No attributes on ZARR group", zg.getAttributes().asScala)
        rawMultiscales <- attrs.get("multiscales")
        ...
        multiscales <- rawMultiscales match {
            ...
            case h :: Nil => safeCast[java.util.HashMap[String, Object]]("multiscales singleton to mapping")(h)
            case multi => s"Multiple (${multi.length}) entries in multiscales list, not just 1".asLeft
        }
        rawAxes <- multiscales.asScala.get("axes").toRight("Missing 'axes' key in multiscales metadata")
        axes <- safeCast[java.util.List[java.util.HashMap[String, String]]]("raw axes to list-of-map")(rawAxes)
