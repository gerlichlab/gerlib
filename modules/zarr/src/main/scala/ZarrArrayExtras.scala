...
object ZarrArrayExtras:
    extension (za: ZarrArray)
        def read(indexMapping: IndexMapping)(origin: OmeZarrStandardCoordinate, size: OmeZarrBlockSize): Either[String, Array[Int]] = 
            val originIndex = indexMapping.buildOriginIndex(origin)
            val sizeIndex = indexMapping.buildSizeIndex(size)
            Try{ JzarrTools.readFrom(za, sizeIndex, originIndex) }
                .toEither
                .leftMap(e => s"For index mapping $indexMapping and starting from $origin, failed to read block of size $size: ${e.getMessage}")
end ZarrArrayExtras
