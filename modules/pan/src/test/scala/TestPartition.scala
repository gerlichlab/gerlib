...
class TestPartition
  ...
  val simpleTypeArbitraries =
    Table("arb", Arbitrary[String], Arbitrary[Int], Arbitrary[Double])
  ...
  def genSmallLegalArgPair[A](g: Gen[A]): Gen[(Int, Set[A])] = for {
    n <- Gen.choose(1, 4) // Number of elements to partition
  ...
  test("Every partition covers the input.") {
    forAll(genSmallLegalArgPair(arbitrary[String])) { (n, xs) =>
      partition(n, xs).forall(_.combineAll === xs) shouldBe true
    }
  }

  test("Every partition consists of disjoint subsets.") {
    forAll(genSmallLegalArgPair(arbitrary[Int])) { (n, xs) =>
      partition(n, xs).forall(_.combinations(2).forall {
        case sub1 :: sub2 :: Nil => (sub1 & sub2).isEmpty
        case subs =>
          throw new Exception(s"${subs.size}, 2, elements when taking pairs!")
      }) shouldBe true
    }
  }
  ...
  test("The number of valid partitions is as expected.") {
    ...
    forAll(genSmallLegalArgPair(arbitrary[Int]), minSuccessful(10)) { (k, xs) =>
      val n = xs.size
      ...
      val numExp = (0 to (k - 1)).map { j =>
        math.pow(-1, j) * (k `choose` j) * math.pow(k - j, n)
      }.sum / factorial(k)
  ...
  test(
    "Requesting more subsets than elements is an IllegalArgumentException."
  ) {
    given noShrink[A]: Shrink[A] = Shrink.shrinkAny[A]
    def genArgs = for {
    ...
    forAll(genArgs) { (k, xs) =>
      ...
      val expMsg =
        s"requirement failed: Desired number of subsets exceeds number of elements: $k > ${xs.size}"
      err.getMessage shouldEqual expMsg
    }
  }

  test(
    "Requesting a non-positive number of subsets is an IllegalArgumentException."
  ) {
    forAll(Gen.choose(Int.MinValue, 0), arbitrary[Set[Int]]) { (k, xs) =>
      val err = intercept[IllegalArgumentException] { partition(k, xs) }
      val expMsg =
        s"requirement failed: Desired number of subsets must be strictly postitive, not $k"
      err.getMessage shouldEqual expMsg
    }
  }
