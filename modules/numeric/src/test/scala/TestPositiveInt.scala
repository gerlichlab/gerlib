...
class TestPositiveInt extends AnyFunSuite, should.Matchers, ScalaCheckPropertyChecks:
    ...
    test("PositiveInt correctly restricts which expressions compile by direct assignment.") {
    ...
    test("PositiveInt is a transparent type alias for Int :| Positive") {
        ...
        assertCompiles{ "val ironRef: Int :| Positive = 1; val aliased: PositiveInt = ironRef" }
        assertCompiles{ "val aliased: PositiveInt = PositiveInt(1); val ironRef: Int :| Positive = aliased" }
    }
end TestPositiveInt
