...
import io.github.iltotore.iron.constraint.numeric.{ Greater, GreaterEqual, Negative }
...
class TestNonnegativeInt extends AnyFunSuite, should.Matchers, ScalaCheckPropertyChecks:
    ...
    test("NonnegativeInt is a transparent type alias for Int :| Not[Negative]") {
        ...
        assertCompiles{ "val ironRef: Int :| Not[Negative] = 0; val aliased: NonnegativeInt = ironRef" }
        assertCompiles{ "val aliased: NonnegativeInt = NonnegativeInt(0); val ironRef: Int :| Not[Negative] = aliased" }
    }
    ...
    test("NonnegativeInt's predicate is Not[Negative], not GreaterEqual[0]") {
        ...
        assertTypeError{ "val ironRef: Int :| GreaterEqual[0] = 0; val aliased: NonnegativeInt = ironRef" }

        /* With Greater[0] | StrictEqual[0] */
        assertCompiles{ "val ironRef: Int :| (Greater[0] | StrictEqual[0]) = 0" }
        assertTypeError{ "val ironRef: Int :| (Greater[0] | StrictEqual[0]) = 0; val aliased: NonnegativeInt = ironRef" }
    }
