...
object extrema:
  ...
  /** Evidence that type {@code A} has a maximum value.
    *
    * @tparam A
    *   The type for which we can generate the maximum value
    */
  ...
  /** Evidence that type {@code A} has a minimum value.
    *
    * @tparam A
    *   The type for which we can generate the minimum value
    */
  ...
  object MaximumValue:
    ...
    extension [A](maxA: MaximumValue[A]) def asBaseTypeValue: A = maxA
  end MaximumValue

  object MinimumValue:
    private[extrema] def apply[A](a: A): MinimumValue[A] = a
    extension [A](minA: MinimumValue[A]) def asBaseTypeValue: A = minA
  end MinimumValue
