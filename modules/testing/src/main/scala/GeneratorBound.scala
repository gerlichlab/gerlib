...
object GeneratorBound:
  ...
  object Lower:
    ...
    def minimum[A](using ev: AdmitsMinimum[A]): Lower[A] = Lower(
      ev.minimum.asBaseTypeValue
    )
  ...
  object Upper:
    ...
    def maximum[A](using ev: AdmitsMaximum[A]): Upper[A] = Upper(
      ev.maximum.asBaseTypeValue
    )
