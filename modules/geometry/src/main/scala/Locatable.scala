 ...
 * @tparam C The type of value wrapped as coordinates for the point
 */
trait Locatable[A, C]:
...
object Locatable:
     ...
     * with coordinates wrapping values of type `C`, and we're told how to get 
     * an `A` from a `B`, then we also know how to get a 3D point wrapping 
     * coordinates of type `C` from any `B`.
     */
    given contravariantForLocatable[C]: Contravariant[[A] =>> Locatable[A, C]] with
