...
class TestCoordinates extends AnyFunSuite, should.Matchers, ScalacheckSuite:
    ...
    test("Coordinates to order cannot be of different wrapper types, even if wrapping the same type.") {
    ...
    test("Coordinate to order cannot be of different underlying value types, even if of the same wrapper type.") {
        /* Positive (no-error) pretests / controls */
        assertCompiles("XCoordinate(1.0) < XCoordinate(2.0)")
        assertCompiles("YCoordinate(-1.0) >= YCoordinate(2.0)")
        assertCompiles("ZCoordinate(1.0) <= ZCoordinate(2.0)")
    ...
    test("Generic coordinates cannot be ordered, even if the same underlying type") {
        ...
        assertTypeError{ "(XCoordinate(1.0): Coordinate[Double]) < (XCoordinate(2.0): Coordinate[Double])" }
        assertTypeError{ "(XCoordinate(1.0): Coordinate[Double]) <= (XCoordinate(2.0): Coordinate[Double])" }
        assertTypeError{ "(XCoordinate(1.0): Coordinate[Double]) > (XCoordinate(2.0): Coordinate[Double])" }
        assertTypeError{ "(XCoordinate(1.0): Coordinate[Double]) >= (XCoordinate(2.0): Coordinate[Double])" }

        /* Failures for runtime-Y */
        assertTypeError{ "(YCoordinate(1): Coordinate[Int]) < (YCoordinate(2): Coordinate[Int])"}
        assertTypeError{ "(YCoordinate(1): Coordinate[Int]) <= (YCoordinate(2): Coordinate[Int])"}
        assertTypeError{ "(YCoordinate(1): Coordinate[Int]) > (YCoordinate(2): Coordinate[Int])"}
        assertTypeError{ "(YCoordinate(1): Coordinate[Int]) >= (YCoordinate(2): Coordinate[Int])"}

        /* Failures for runtime-Z */
        assertTypeError{ "(ZCoordinate(2.0): Coordinate[Double]) < (ZCoordinate(3): Coordinate[Double])" }
        assertTypeError{ "(ZCoordinate(2.0): Coordinate[Double]) <= (ZCoordinate(3): Coordinate[Double])" }
        assertTypeError{ "(ZCoordinate(2.0): Coordinate[Double]) > (ZCoordinate(3): Coordinate[Double])" }
        assertTypeError{ "(ZCoordinate(2.0): Coordinate[Double]) >= (ZCoordinate(3): Coordinate[Double])" }
    }
